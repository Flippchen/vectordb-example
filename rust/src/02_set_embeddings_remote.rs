...
fn get_embeddings(titles: &[String]) -> Vec<Vec<f32>> {
    // Set-up sentence embeddings model
    let model = SentenceEmbeddingsBuilder::remote(SentenceEmbeddingsModelType::BertBaseNliMeanTokens)
        .create_model().unwrap();
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for chunk in all_keys.chunks(1000) {
        ...
        let embeddings = get_embeddings(&titles);
